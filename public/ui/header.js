...
export function initHeader() {
  ...
  if (!document.querySelector('.nkm-header-search')) {
    ...
    searchSlot.innerHTML = `<div class="nkm-search-wrap"><input class="nkm-search" id="nkmHeaderSearch" placeholder="Search name, NKM ID…" aria-label="Search"></div><div id="nkmHeaderSearchResults" style="position:absolute; top:42px; left:0; right:0; background:#141b2e; border:1px solid rgba(148,163,184,.12); border-radius:12px; padding:8px; display:none; max-height:60vh; overflow:auto; z-index:70"></div>`;
  ...
  // Mobile search button (opens sheet)
  if (!document.querySelector('.nkm-header-search-btn')) {
    ...
    btn.addEventListener('click', () => {
      const { body } = createSheet({ title: 'Search', content: `<div class="nkm-search-wrap"><input class="nkm-search" id="nkmSheetSearch" placeholder="Search…" autocomplete="off"><div id="nkmSheetResults" style="margin-top:12px"></div></div>` });
      const input = body.querySelector('#nkmSheetSearch');
      const results = body.querySelector('#nkmSheetResults');
