...
import{mechanicsFor}from'./forge-mechanics.js';
import{escapeHtml as esc}from'./utils.js';
import{showToast}from'./ui/toast.js';

const $=id=>document.getElementById(id);
let me=null,profile=null,activeType=new URLSearchParams(location.search).get('activity')||'puzzle',selected,selectedMechanic=null;
...
function renderLanes(){
 ...
 document.querySelectorAll('.creator-lane').forEach(b=>b.onclick=()=>{activeType=b.dataset.type;renderLanes();renderCategorySuggestions();renderMechanics();renderTemplates()});
}
...
function renderMechanics(){
 const list=mechanicsFor(activeType);
 if(!selectedMechanic||!list.some(x=>x.id===selectedMechanic))selectedMechanic=list[0]?.id||null;
 $('creatorMechanics').innerHTML=list.map(m=>'<button type="button" class="creator-mechanic '+(m.id===selectedMechanic?'selected':'')+'" data-mechanic="'+m.id+'"><span class="creator-mechanic-icon">'+m.icon+'</span><span><strong>'+escText(m.label)+'</strong><small>'+escText(m.desc)+'</small></span></button>').join('');
 document.querySelectorAll('[data-mechanic]').forEach(b=>b.onclick=()=>{selectedMechanic=b.dataset.mechanic;renderMechanics();renderLivePreview()});
}
function renderTemplates(){
 ...
 document.querySelectorAll('.creator-template').forEach(b=>b.onclick=()=>{if(b.dataset.id==='__blank__'){selected=null;selectedMechanic=mechanicsFor(activeType)[0]?.id||null;$('title').value='';$('category').value='';$('description').value='';$('goal').value='';$('instructions').value='';$('durationMin').value='20';$('difficulty').value='Medium';$('expiresDays').value='14';$('xpReward').value='40';$('icon').value=ACTIVITY_TYPES[activeType].icon;renderMechanics();updateExpiry()}else{selected=ACTIVITY_CATALOG.find(t=>t.id===b.dataset.id)||null;fillForm(selected)}renderTemplates()});
 if(selected===undefined||(selected&&selected.type!==activeType)){selected=list[0]||null;if(selected)fillForm(selected)}
}
function fillForm(t){
 if(!t)return;
 $('title').value=t.title||'';$('category').value=t.category||'';$('description').value=t.description||'';$('goal').value=t.goal||'';$('instructions').value=t.instructions||'';$('durationMin').value=String(t.durationMin||20);$('difficulty').value=t.difficulty||'Medium';$('expiresDays').value=String(t.cycleDays||14);selectedMechanic=t.mechanic||mechanicsFor(activeType)[0]?.id||null;$('xpReward').value=t.difficulty==='Hard'?'60':t.difficulty==='Medium'?'40':'25';$('icon').value=t.icon||ACTIVITY_TYPES[activeType].icon;
 updateExpiry();
}
