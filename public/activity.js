...
const ref=doc(db,'users',me.uid,'activityCompletions',cycleKey);const old=await getDoc(ref);if(old.exists()){$('completionNote').textContent='Already completed in this cycle ✓';b.textContent='Completed';return}await setDoc(ref,{uid:me.uid,activityId:activity.id,cycleKey,title:activity.title,type:activity.type,completedAtMs:Date.now(),cycleEndsAtMs:activity.endAtMs});const xp=activity.difficulty==='Hard'?60:activity.difficulty==='Medium'?40:25;const award=await awardXp(me.uid,xp,{catalogActivityId:activity.id});showAchievement({title:activity.title,subtitle:'+'+xp+' XP',icon:activity.icon||'🎯',leveledUp:award?.leveledUp,level:award?.level,badges:award?.badgesEarned||[]});$('completionNote').innerHTML='<div class="activity-success">Completed ✓ Great job. You can rate community activities after finishing them.</div>';b.textContent='Completed'}catch(e){b.disabled=false;b.textContent='Mark complete';$('completionNote').textContent=e.message||'Could not save completion.'}};
